...
/// The [InternetBankingController] manages the state and logic for
/// creating a internet banking source from Omise API.
class InternetBankingController
    extends ValueNotifier<InternetBankingPageState> {
  /// Instance of [OmiseApiService] used to interact with the omise dart package.
  final OmiseApiService omiseApiService;

  /// Constructor for initializing [InternetBankingController].
  /// Takes in a required [omiseApiService].
  InternetBankingController({
    required this.omiseApiService,
  }) : super(InternetBankingPageState(sourceLoadingStatus: Status.idle));
  ...
  void _setValue(InternetBankingPageState state) {
    value = state;
  }
}

/// State class that holds the values for [InternetBankingController].
class InternetBankingPageState {
  ...
  /// Constructor for creating a [InternetBankingPageState].
  InternetBankingPageState({
  ...
  InternetBankingPageState copyWith({
    Source? source,
    ...
    return InternetBankingPageState(
