...
void main() {
  ...
  late MobileBankingPaymentMethodSelectorController mockController;

  setUp(() {
    mockOmiseApiService = MockOmiseApiService();
    mockController = MockPMobileBankingPaymentMethodSelectorController();
  });
  ...
  group("SelectMobileBankingPaymentMethodPage", () {
    testWidgets('displays list of mobile banking payment methods',
      ...
      when(() => mockController.value)
          .thenReturn(MobileBankingPaymentMethodSelectorState(
        sourceLoadingStatus: Status.idle,
      ));

      await tester.pumpWidget(
        MaterialApp(
          home: SelectMobileBankingPaymentMethodPage(
    ...
    testWidgets(
      ...
      (WidgetTester tester) async {
                ...
                return ElevatedButton(
                  onPressed: () async {
                    final result = await Navigator.of(context).push(
                      MaterialPageRoute(
                        builder: (context) =>
                            SelectMobileBankingPaymentMethodPage(
                          omiseApiService: mockOmiseApiService,
                          amount: amount,
                          currency: currency,
        ...
        expect(find.byType(SelectMobileBankingPaymentMethodPage), findsNothing);
      },
    );
  });
}
